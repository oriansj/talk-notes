...
hardware target (encoding can be expressed in human readable form) and provide
the documentation and testing required.
...
+ We completely agree; however writing 79,000+ custom bootstraps isn't viable.
+ Simply implement the stage0 virtual machine via TTL logic or individual
transistors to be absolutely sure that the hardware basis of the bootstrap was
not compromised.
